...
import type { Orden, EstadoOrden, ESTADO_ORDER } from "@/types";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchOrdenes = async () => {
      const { data } = await supabase
        .from("ordenes")
        .select("*")
        .in("estado", ["ingresado", "en_diagnostico", "esperando_repuesto", "en_reparacion", "listo_para_retiro"])
  ...
  const stats = {
    ...
    enProceso: ordenes.filter(o => !["ingresado", "listo_para_retiro"].includes(o.estado)).length,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard
          ...
          value={stats.total}
          icon={
            <svg className="w-6 h-6 text-ink-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <StatsCard
          ...
          icon={
            <svg className="w-6 h-6 text-status-red" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <StatsCard
          ...
          variant="warning"
          icon={
            <svg className="w-6 h-6 text-status-yellow" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <StatsCard
          ...
          icon={
            <svg className="w-6 h-6 text-status-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {ordenes.slice(0, 5).map(orden => {
                const semaphore = getSemaphoreColor(orden.updated_at, orden.es_urgente);
                return (
                  <Link
                    key={orden.id}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-ink-primary">{orden.falla_declarada}</p>
